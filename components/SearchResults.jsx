...
export default function SearchResults({ results, setViewport, setSelectedLocation }) {
    ...
    <ResultsDiv className='hero'>
      <div className='inner'>
        <p className='details'>
          ...
          <span className='date'>{checkOutDate}</span> for {router.query.guests} guests
        </p>
        ...
        <div className='results'>
          {results.map((item, index) => (
            <ResultCard
              onClick={() => setSelection({ lat: item.lat, long: item.long, index: index })}
              {...item}

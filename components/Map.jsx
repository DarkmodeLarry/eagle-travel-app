...
export default function Map({
  ...
  const handleScroll = () => {
    if (window.scrollY < window.innerHeight * 1.2) {
      containerRef.current.style.transform = 'translateY(' + window.scrollY * 0.2 + 'px)'
    }
  }
  ...
  return (
    <MapContainer ref={containerRef}>
      <ReactMapGL
        ...
        mapboxApiAccessToken='pk.eyJ1Ijoic3BpZGVyYmVhciIsImEiOiJjbGJ4a3lwcTMwcDN3M25wcXB0cm51eG90In0.R1U-0B45S6HtimMYZ5iOjQ'
        {...viewport}
        width='100%'
        height='100%'
        ...
        {results.map((result, index) => (
          <div key={result.long}>
            <Marker longitude={result.long} latitude={result.lat}>
              <p
                role='img'
                className={`mapMarker ${selectedLocation.long === result.long ? 'active' : null}`}
